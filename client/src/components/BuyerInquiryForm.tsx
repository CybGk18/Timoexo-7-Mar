...
const BuyerInquiryForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/buyer_inquiry.php`, {
        method: 'POST',
        headers: {
      ...
      const result = contentType.includes('application/json')
        ? JSON.parse(rawText)
        : { success: false, error: rawText || `Server returned ${response.status} ${response.statusText}` };

      if (response.ok && result.success) {
        alert('Buyer inquiry submitted successfully!');
        // Reset form
        setFormData({
          fullName: '',
          email: '',
        ...
        alert(result.error || result.message || 'Failed to submit inquiry. Please try again.');
      }
    } catch (error) {
  ...
  return (
    <div className="p-4 sm:p-5 text-sm">
      <div className="mb-4">
        <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">Buyer Inquiry</h2>
        <p className="text-gray-600 text-xs sm:text-sm">
          Tell us what you're looking for. We'll connect you with reliable manufacturers.
        </p>
      </div>
